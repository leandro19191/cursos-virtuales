...
class atto_reciteditor_external extends external_api {
    ...
    public static function import_templates($fileContent) {
        ...
        foreach($fileContent as $item){
            ...
            $DB->insert_record('atto_reciteditor_templates', array('name' => $item->name, 'type' => $item->type, 'userid' => $USER->id, 'htmlstr' => $item->htmlStr, 'img' => $item->img));
        }
        return array('success' => true);
    }
